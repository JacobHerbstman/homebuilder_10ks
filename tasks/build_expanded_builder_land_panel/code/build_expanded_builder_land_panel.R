...
if (nrow(balanced_panel) != 858L || balanced_panel |> count(universe_episode_id, fiscal_year) |> filter(n != 1L) |> nrow() > 0L) {
  stop("Expanded builder land panel is not unique and complete for 39 episodes over 2004-2025.")
}

if (balanced_panel |>
    filter(!is.na(selected_nonowned_controlled_share) &
      (selected_nonowned_controlled_share < 0 | selected_nonowned_controlled_share > 1)) |>
    nrow() > 0L) {
  stop("Expanded builder land panel has a selected share outside zero and one.")
}
...
cat("Wrote expanded builder land panel and plot data to ../output\n")
