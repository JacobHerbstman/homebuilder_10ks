...
VARIABLE_FAMILIES = {
    ...
    "deposits_and_preacquisition_costs": "option_economics",
...
def candidate_rows_for_filing(base, text, source_path, fallback_scale_factor=1):
    ...
    for match in VARIABLE_MASTER_PATTERN.finditer(text):
        ...
        values = candidate_values(snippet, term_start_in_snippet, expected_unit, fallback_scale_factor)
...
def detect_table_scale(matrix, context):
    table_text = clean_key(" ".join([" ".join(row) for row in matrix[:6]]) + " " + context)
    if re.search(r"\$000|000 s omitted|000s omitted|thousands?\b|\(\$000", table_text):
        return "$000s omitted", 1000
    if re.search(r"millions?\b", table_text):
...
def row_labels(matrix, row_index):
    ...
    for prior_index in range(row_index - 1, max(-1, row_index - 12), -1):
        ...
        if not prior_label:
            continue
        if (
            prior_label.endswith(":")
            or not row_has_number(prior)
            or re.search(r"net new orders|backlog|valuation adjustments|net realizable value|write off|write offs|impairments of investments|land related charges", prior_key)
        ):
...
def source_scope_for(row_label, combined_label, variable_name):
    ...
    if VARIABLE_FAMILIES.get(variable_name) == "inventory_accounting" and re.search(
        r"homes under construction|land under development|land held for future development|"
        r"land held for sale|total inventory|total assets",
        key,
    ):
        return "firm_year", ""
...
def classify_table_metric(row_label, combined_label, column_label, table_key):
    ...
    if re.search(r"total land lots?|total lots?|owned and controlled", col_key):
        return "total_lots"
    if "years of supply" in col_key:
        return ""
    if re.search(r"land lots? owned|lots? owned|\bowned\b", col_key) and "not owned" not in col_key:
        return "owned_lots"
    if re.search(r"\boptioned\b|option lots?", col_key):
        return "optioned_lots"
    if re.search(r"lots? controlled|controlled lots?|\bcontrolled\b", col_key) and "not owned" not in col_key:
    ...
    if re.search(r"total inventory", col_key):
        return "total_inventory"
    if re.search(r"total assets", col_key):
        return "total_assets"
    if re.search(r"land held for future development", col_key):
        return "land_held_for_future_development"
    if re.search(r"land under development", col_key):
        return "land_under_development"
    if re.search(r"homes under construction", col_key):
        return "homes_under_construction_inventory"

    if re.search(r"homes under construction", combo_key):
        return "homes_under_construction_inventory"
    if re.search(r"land under development", combo_key):
        return "land_under_development"
    if re.search(r"land held for future development", combo_key):
        return "land_held_for_future_development"
    if re.search(r"total inventory", combo_key):
        return "total_inventory"
    if re.search(r"total assets", combo_key):
        return "total_assets"
    ...
    if re.search(r"land not owned under option agreements?", combo_key) or re.search(r"liabilities for land not owned", combo_key):
        return "land_not_owned_under_option_agreements"
    if re.search(r"non refundable deposits? and pre acquisition costs?|nonrefundable deposits? and pre acquisition costs?|non refundable deposits?|nonrefundable deposits?", combo_key):
    ...
    ) and not re.search(r"write offs?|write down|land related", row_key):
        return "deposits_preacquisition_costs"
    if re.search(r"refundable deposits?|refundable portion", combo_key):
...
def table_candidates_for_filing(base, source_path):
    ...
    for table_index, table in enumerate(parse_html_tables(source_path), start=1):
        ...
        for row_index, row in enumerate(matrix):
            ...
            row_is_percent = "%" in row_text and (
                clean_key(row_label) == "total" or "%" in clean_key(row_label)
            )
            ...
            for col_index in range(len(row)):
                ...
                variable_name = classify_table_metric(row_label, combined_label, column_label, table_key)
                ...
                if unit == "dollars":
                    if variable_name == "average_selling_price":
                        applied_scale = 1000 if value < 10000 else 1
                    else:
                        applied_scale = table_scale_factor
                    value *= applied_scale

                source_scope, segment_label = source_scope_for(row_label, combined_label, variable_name)
