...
for (i in seq_len(nrow(inventory))) {
  ...
  if (filing$ticker == "TMHC") {
    ...
    source_table_text <- paste(na.omit(c(orders_text, operating_text, deliveries_text, backlog_text)), collapse = " || ")
  ...
  if (filing$ticker == "TPH") {
    ...
    orders_prose <- NA_character_
    if (is.na(orders_units)) {
      orders_prose <- str_extract(
        filing_text,
        regex("Net new home orders for the year ended.*?(?:increased|decreased).*?to\\s+[0-9,]+", ignore_case = TRUE)
      )
      orders_units <- parse_number(str_match(orders_prose, regex("to\\s+([0-9,]+)", ignore_case = TRUE))[, 2])
    }
    ...
    source_table_text <- paste(na.omit(c(orders_text, orders_prose, deliveries_text, backlog_text)), collapse = " || ")
    if (source_table_text == "") source_table_text <- NA_character_
  ...
  if (filing$ticker == "GRBK") {
    ...
    deliveries_text <- table_text[deliveries_index[1]]
    orders_text <- table_text[orders_index[1]]
    deliveries_value_thousands <- parse_number(str_match(deliveries_text, regex("Home closings revenue(?: \\(dollars in thousands\\))?\\s+\\$\\s*([0-9,]+)", ignore_case = TRUE))[, 2])
    ...
    if (filing$fiscal_year != 2018) average_selling_price_dollars <- 1000 * average_selling_price_dollars
  ...
  if (filing$ticker %in% c("DFH", "SDHC", "UHG")) {
    ...
    active_communities <- parse_number(str_match(
      summary_text,
      regex("Active communities(?: at end of period| as of period[- ]end| \\(period end\\))\\s*(?:\\([0-9a-z]+\\))?\\s+([0-9,]+)", ignore_case = TRUE)
    )[, 2])
    source_table_text <- summary_text
...
if (panel |> filter(is.na(orders_units) | is.na(deliveries_units) | is.na(backlog_units)) |> nrow() > 0) {
  stop("Nine-firm annual operating panel has a missing core operating count.")
}

if (panel |> filter(!is.na(active_communities) & (active_communities <= 0 | active_communities > 5000)) |> nrow() > 0) {
  stop("Nine-firm annual operating panel has an implausible active-community count.")
}
