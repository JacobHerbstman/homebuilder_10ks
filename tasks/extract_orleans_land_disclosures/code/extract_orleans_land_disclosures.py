#!/usr/bin/env python3

import csv
import re
import sys
from pathlib import Path

sys.path.append(str(Path(__file__).resolve().parents[2] / "_lib"))
from sec_fetch_utils import write_csv

sys.path.append(str(Path(__file__).resolve().parents[1].parent / "extract_10k_land_candidates" / "code"))
from extract_10k_land_candidates import SecTableParser, clean_text, visible_text


with open("../input/land_light_firm_year_measures.csv", newline="") as f:
    filing_rows = [
        row for row in csv.DictReader(f)
        if row.get("ticker") == "OHB"
        and row.get("form") == "10-K"
        and 2004 <= int(float(row.get("fiscal_year"))) <= 2008
    ]

if len(filing_rows) == 0:
    raise RuntimeError("No Orleans filing rows found in land_light_firm_year_measures.csv.")

filing_by_year = {}
for filing in filing_rows:
    fiscal_year = int(float(filing["fiscal_year"]))
    if fiscal_year in filing_by_year:
        raise RuntimeError("Orleans filing rows must be unique by fiscal_year.")
    filing_by_year[fiscal_year] = filing

expected_values = {
    2004: (3721, 8570, 12291),
    2005: (7451, 9690, 17141),
    2006: (8637, 8241, 16878),
    2007: (7849, 2654, 10503),
    2008: (5382, 1847, 7229),
}

panel_rows = []
source_notes = []

for fiscal_year in range(2004, 2009):
    filing = filing_by_year[fiscal_year]
    source_path = Path(filing["primary_document_local_path"])

    if not source_path.exists():
        raise RuntimeError(f"Missing Orleans source file: {source_path}")

    raw_text = source_path.read_text(encoding="utf-8", errors="ignore")
    source_text = visible_text(source_path)

    owned_lots = None
    controlled_lots = None
    total_lots = None
    source_table_index = ""
    source_row_label = ""
    source_excerpt = ""

    if fiscal_year == 2005:
        section_match = re.search(
            r"The following table sets forth the Company's land positions as of June 30, 2005\.(.*?)CUSTOMER SERVICE",
            raw_text,
            re.S,
        )
        if section_match:
            total_match = re.search(
                r"Total\.+\s+([\d,]+)\s+([\d,]+)\s+([\d,]+)",
                section_match.group(1),
            )
            if total_match:
                owned_lots = float(total_match.group(1).replace(",", ""))
                controlled_lots = float(total_match.group(2).replace(",", ""))
                total_lots = float(total_match.group(3).replace(",", ""))
                source_row_label = "Total"
                source_excerpt = clean_text(section_match.group(1))[:2200]
    else:
        table_parser = SecTableParser()
        table_parser.feed(raw_text)

        for table_index, table in enumerate(table_parser.tables):
            parsed_rows = []
            for html_row in table["rows"]:
                cells = [clean_text(cell.get("text", "")) for cell in html_row]
                cells = [cell for cell in cells if cell]
                if cells:
                    parsed_rows.append(cells)

            table_text = clean_text(" || ".join(" | ".join(cells) for cells in parsed_rows))
            table_text_lower = table_text.lower()
            if "controlled" not in table_text_lower:
                continue
            if "option" not in table_text_lower and "agreement of sale" not in table_text_lower:
                continue

            for cells in parsed_rows:
                if len(cells) < 4 or cells[0] != "Total":
                    continue
                integer_cells = [
                    float(cell.replace(",", ""))
                    for cell in cells
                    if re.fullmatch(r"\d+(?:,\d{3})*", cell)
                ]
                if len(integer_cells) >= 3:
                    owned_lots = integer_cells[0]
                    controlled_lots = integer_cells[1]
                    total_lots = integer_cells[2]
                    source_table_index = str(table_index)
                    source_row_label = "Total"
                    source_excerpt = table_text[:2200]
                    break

            if total_lots is not None:
                break

    if total_lots is None:
        raise RuntimeError(f"Could not extract Orleans land-position values for fiscal {fiscal_year}.")

    post_year_end_acquisition_proforma_lots = ""
    if fiscal_year == 2004:
        realen_match = re.search(
            r"including the acquisition of realen homes.*?total number of lots owned or controlled.*?([\d,]+)",
            source_text,
            re.I,
        )
        if realen_match:
            post_year_end_acquisition_proforma_lots = float(realen_match.group(1).replace(",", ""))

    component_identity_gap = owned_lots + controlled_lots - total_lots

    panel_rows.append({
        "ticker": "OHB",
        "cik10": filing["cik10"],
        "sec_company_name": filing["sec_company_name"],
        "fiscal_year": fiscal_year,
        "report_date": filing["report_date"],
        "filing_date": filing["filing_date"],
        "accession_number": filing["accession_number"],
        "primary_document": filing["primary_document"],
        "source_local_path": filing["primary_document_local_path"],
        "source_url": filing["filing_url"],
        "unit_type": "lots",
        "owned_lots": owned_lots,
        "nonowned_controlled_lots": controlled_lots,
        "optioned_lots": controlled_lots,
        "total_lots": total_lots,
        "nonowned_controlled_share": controlled_lots / total_lots,
        "optioned_share": controlled_lots / total_lots,
        "owned_share": owned_lots / total_lots,
        "component_identity_gap": component_identity_gap,
        "component_identity_pass": abs(component_identity_gap) <= 1,
        "extraction_method": "orleans_lots_owned_option_controlled_total_table",
        "source_table_index": source_table_index,
        "source_row_label": source_row_label,
        "panel_use_flag": True,
        "manual_review_flag": False,
        "manual_review_reason": "",
        "post_year_end_acquisition_proforma_lots": post_year_end_acquisition_proforma_lots,
        "source_note": (
            "Orleans nonowned numerator is the recurring firm-level lots under option agreement, "
            "agreement of sale, or contracted-to-purchase-or-under-option count, so it is broader "
            "than strict pure cancellable options. Undeveloped-land-only MD&A counts are not used "
            "as the main omega numerator because the firm-level total includes both undeveloped land "
            "and improved lots."
        ),
    })

    source_notes.append({
        "ticker": "OHB",
        "fiscal_year": fiscal_year,
        "source_table_index": source_table_index,
        "source_row_label": source_row_label,
        "source_excerpt": source_excerpt,
        "source_note": (
            "FY2004 uses the June 30 table and excludes the post-year-end Realen pro forma lot position. "
            if fiscal_year == 2004 else ""
        ),
    })

audit_rows = []
for row in panel_rows:
    expected_owned, expected_controlled, expected_total = expected_values[row["fiscal_year"]]
    audit_rows.append({
        "audit_check": "expected_values",
        "fiscal_year": row["fiscal_year"],
        "status": "ok" if row["owned_lots"] == expected_owned and row["nonowned_controlled_lots"] == expected_controlled and row["total_lots"] == expected_total else "fail",
        "value": f'{row["owned_lots"]}|{row["nonowned_controlled_lots"]}|{row["total_lots"]}',
        "detail": f"Expected {expected_owned}|{expected_controlled}|{expected_total}.",
    })

audit_rows.append({
    "audit_check": "firm_year_rows",
    "fiscal_year": "",
    "status": "ok" if len(panel_rows) == 5 else "fail",
    "value": len(panel_rows),
    "detail": "Expected Orleans fiscal 2004-2008.",
})

audit_rows.append({
    "audit_check": "component_identity",
    "fiscal_year": "",
    "status": "ok" if all(row["component_identity_pass"] for row in panel_rows) else "fail",
    "value": sum(row["component_identity_pass"] for row in panel_rows),
    "detail": "Owned lots plus nonowned controlled lots must equal total lots.",
})

write_csv(
    "../output/ohb_2004_2008_land_panel.csv",
    panel_rows,
    [
        "ticker", "cik10", "sec_company_name", "fiscal_year", "report_date",
        "filing_date", "accession_number", "primary_document", "source_local_path",
        "source_url", "unit_type", "owned_lots", "nonowned_controlled_lots",
        "optioned_lots", "total_lots", "nonowned_controlled_share", "optioned_share",
        "owned_share", "component_identity_gap", "component_identity_pass",
        "extraction_method", "source_table_index", "source_row_label",
        "panel_use_flag", "manual_review_flag", "manual_review_reason",
        "post_year_end_acquisition_proforma_lots", "source_note",
    ],
)

write_csv(
    "../output/ohb_2004_2008_extraction_audit.csv",
    audit_rows,
    ["audit_check", "fiscal_year", "status", "value", "detail"],
)

write_csv(
    "../output/ohb_2004_2008_source_notes.csv",
    source_notes,
    ["ticker", "fiscal_year", "source_table_index", "source_row_label", "source_excerpt", "source_note"],
)
